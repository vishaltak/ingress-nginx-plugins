...
describe("main", function()
  describe("rewrite", function()
    it("sets x-hello-world header to 'hello' when user agent is hello", function()
      ngx.var = { http_user_agent = "hello" }
      stub(ngx.req, "set_header")
      main.rewrite()
      assert.stub(ngx.req.set_header).was_called_with("x-hello-world", "hello")
    end)

    it("sets x-hello-world header to 'unknown' when user agent is not hello", function()
      ngx.var = { http_user_agent = "not-hello" }
      stub(ngx.req, "set_header")
      main.rewrite()
      assert.stub(ngx.req.set_header).was_called_with("x-hello-world", "unknwon")
    end)
  end)
end)
